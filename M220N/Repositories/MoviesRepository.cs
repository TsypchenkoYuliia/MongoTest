...
namespace M220N.Repositories
{
    public class MoviesRepository
    {
        ...
        public async Task<Movie> GetMovieAsync(string movieId, CancellationToken cancellationToken = default)
        {
            ...
            //var test = (await _moviesCollection.FindAsync(Builders<Movie>.Filter.Eq("Id", movieId))).FirstOrDefault();
            //test.Comments = (await _commentsCollection.FindAsync(Builders<Comment>.Filter.Eq("movie_id", objectId))).ToList();
            ...
            try
            {
                return await _moviesCollection.Aggregate()
                    ...
                    m=>m.Id,
                    c=>c.MovieId,
                    (Movie m) => m.Comments.OrderByDescending(c=>c.Date)
                    )
